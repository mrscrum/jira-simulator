...
def test_store_bootstrap_is_idempotent_and_reloads_detached_state(v2_session_factory):
    ...
    first = store.ensure_bootstrapped(aggregate.team.id, STARTED_AT)
    second = store.ensure_bootstrapped(aggregate.team.id, STARTED_AT)
    ...
    assert reloaded.aggregate.runtime.simulation_time == STARTED_AT
    assert reloaded.aggregate.runtime.next_wake_at == STARTED_AT
